...
# Assignment 04 - Four in sequence
# A CLI version of Connect 4 that supports both Player vs Player and Player vs CPU games
...
def getPlayerPiece(playerNumber: int) -> str:
...
def createBoard(width: int, height: int) -> list[list[str]]:
...
def getColumnInt(board: list[list[str]], playerNumber: int):
...
def getInputInRange(board: list[list[str]], playerNumber: int) -> int:
...
def getHumanInput(board: list[list[str]], playerNumber: int) -> int:
...
def checkForNextMoveWin(board: list[list[str]], playerNumber: int) -> int:
...
def checkAdjacent(board: list[list[str]], playerNumber: int) -> int:
        ...
        if row != -1:
            ...
            if column - 1 >= 0:
                if board[row][column-1] == piece:
                    adjacents.append(column)
            if row + 1 <= len(board) - 1 and column - 1 >= 0:
                if board[row+1][column-1] == piece:
                    adjacents.append(column)
            if row + 1 <= len(board) - 1 :
                if board[row+1][column] == piece:
                    adjacents.append(column)
            if row + 1 <= len(board) - 1 and column + 1 <= len(board[0]) - 1:
                if board[row+1][column+1] == piece:
                    adjacents.append(column)
            if column + 1 <= len(board[0]) - 1:
                if board[row][column+1] == piece:
                    adjacents.append(column)
            if row - 1 >= 0 and column + 1 <= len(board[0]) - 1:
                if board[row-1][column+1] == piece:
                    adjacents.append(column)
...
def getComputerInput(board: list[list[str]], currentPlayerTurn: int) -> int:
    ...
    Finally: If the third step fails, meaning that there are not any moves that would result in at least two pieces connecting, then just pick a random column and end the turn.
...
def getOpenRow(board: list[list[str]], col: int) -> int:
...
def placePiece(board: list[list[str]], row: int, col: int, piece: str) -> None:
...
def dropPieceIntoBoard(board: list[list[str]], col: int, playerNumber: int) -> None:
...
def checkDraw(board: list[list[str]]) -> bool:
...
def checkWinner(board: list[list[str]], playerNumber: int) -> bool:
...
def resetGameOptions() -> tuple[int, bool, bool]:
...
def switchTurns(currentPlayerTurn: int) -> int:
...
def main() -> None:
    ...
    while running:
        ...
        if choice == "p":
            ...
            print("RULES:")
            print("Your goal is to get four of your pieces into either a row, column, or a diagonal")
            print("On your turn, enter the number of the corosponding row drop a token there")
            print("Be careful though: the computer isn't stupid...")
        elif choice == "q":
            running = False
